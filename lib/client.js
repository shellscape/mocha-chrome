...
module.exports = async function connectClient (instance, log, options) {
  ...
  await Promise.all([
    ...
    // @shellscape perhaps include a hook for other CDP setup here? eg: Promise.resolve(typeof options.setupChromeDebugging === 'function' && options.setupChromeDebugging(client))
  ]);
